...
const forgotEmailValid = async (req, res) => {
  try {
    ...
    if (findUser) {
      ...
      res.render("forgot-password", {
        message: "Email does not match, Please confirm email!",
      });
    }
...
const verifyForgotPassOtp = async (req, res) => {
  try {
    ...
    if (enteredOtp === req.session.userOtp) {
      res.status(OK).json({ success: true, redirectUrl: "/reset-password" });
    } else {
      res.status(BadRequest).json({ success: false, message: "OTP is not match" });
    }
  } catch (error) {
...
const resendOtp = async (req, res) => {
  try {
    ...
    if (emailSent) {
      ...
      res
        .status(OK)
        .json({ success: true, message: "Resend OTP seccessfull" });
    }
  } catch (error) {
    console.error("Error in resend otp", error);
    res.status(InternalServerError).json({ success: false, message: "Internal server error" });
  }
};
  ...
  try {
    ...
    if (userExists && userExists.email === email) {
      ...
      res.render("change-email", {
        message: "Email not found!, Please verify email id",
      });
    }
  } catch (error) {
...
const verifyNewEmail = async (req, res) => {
  try {
    ...
    if (req.session.userOtp === enteredOtp) {
      await User.findByIdAndUpdate(user, { email: newEmail }, { new: true });
...
const deleteAddress = async (req, res) => {
  try {
    ...
    if (!findAddress) {
      return res.status(NotFound).send("Address not found");
    }
