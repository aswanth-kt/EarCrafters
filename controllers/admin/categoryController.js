...
const categoryInfo = async (req, res) => {
  try {
    ...
    res.status(InternalServerError).redirect("/pageerror");
  }
};
...
const addCategory = async (req, res) => {
  try {
    ...
    if (existingCategory) {
      return res.status(BadRequest).json({ error: "Category already exists." });
    }
    ...
    return res.status(InternalServerError).json({ error: "Internal server error." });
  }
};
...
const editCategory = async (req, res) => {
  try {
    ...
    if (updatedCategory) {
      ...
      return res.status(BadRequest).json({ error: "Category not found" });
    }
  } catch (error) {
